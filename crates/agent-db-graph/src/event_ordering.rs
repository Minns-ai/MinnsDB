...
impl EventOrderingEngine {
    ...
    pub async fn process_event(&self, event: Event) -> GraphResult<OrderingResult> {
        ...
        // Step 1: Add event to agent-specific buffer
        {
            ...
            if let Some(last_ts) = agent_buffer.last_processed_timestamp {
                if event.timestamp < last_ts {
                    ...
                    if delay > self.config.reorder_window_ms * 1_000_000 { // Convert to nanoseconds
        ...
        if self.config.strict_causality {
            result.issues.extend(self.check_causality_violations(&event).await?);
        }
        
        // Step 5: Update buffer counts
        {
            let buffers = self.agent_buffers.read().await;
            result.buffered_count = buffers.values()
                .map(|buf| buf.pending_events.len())
                .sum();
        }
    ...
    async fn try_flush_agent_buffer(&self, agent_id: AgentId) -> GraphResult<Vec<Event>> {
        ...
        if let Some(agent_buffer) = buffers.get_mut(&agent_id) {
            ...
            for (&timestamp, event) in &agent_buffer.pending_events {
                ...
                let is_ready = age >= reorder_window_ns || 
                    self.is_next_in_sequence(agent_buffer, event);
    ...
    pub async fn get_stats(&self) -> OrderingStats {
        ...
        let total_buffered: usize = buffers.values()
            .map(|buf| buf.pending_events.len())
            .sum();
        
        let total_processed: u64 = global_buffer.timeline.values()
            .map(|events| events.len() as u64)
            .sum();
        
        let total_reordered: u64 = buffers.values()
            .map(|buf| buf.stats.events_reordered)
            .sum();
    ...
    fn get_oldest_buffered_timestamp(&self, buffers: &HashMap<AgentId, AgentEventBuffer>) -> Option<Timestamp> {
        buffers.values()
