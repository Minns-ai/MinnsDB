...
use crate::structures::{NodeId, NodeType};
use crate::{GraphError, GraphResult};
use agent_db_core::types::{current_timestamp, AgentType, SessionId};
...
impl ScopedInferenceEngine {
    ...
    pub async fn get_scope_engine(
        &self,
        scope: &InferenceScope,
    ) -> GraphResult<Arc<RwLock<GraphInference>>> {
    ...
    pub async fn process_scoped_event(
        &self,
        scoped_event: ScopedEvent,
    ) -> GraphResult<ScopedInferenceResult> {
        ...
        let ordering_result = ordering_engine
            .process_event(scoped_event.event.clone())
            .await?;
        ...
        if self.config.enable_cross_scope_patterns {
            let cross_patterns = self
                .detect_cross_scope_patterns(&scope, &scoped_event)
                .await;
        ...
        match query {
            ScopeQuery::AgentCollaboration => {
                ...
                let collaboration_patterns = self
                    .find_agent_collaboration_patterns(&inference_engine)
                    .await?;
                Ok(ScopeQueryResult::CollaborationPatterns(
    ...
    pub async fn cleanup_inactive_scopes(&self) -> GraphResult<CleanupReport> {
        ...
        let cutoff_time =
            current_timestamp() - (self.config.session_timeout_hours * 3600 * 1_000_000_000);
    ...
    async fn get_or_create_scope_engine(
        &self,
        scope: &InferenceScope,
    ) -> GraphResult<Arc<RwLock<GraphInference>>> {
        {
        ...
        if engines.len() >= self.config.max_scopes {
            return Err(GraphError::OperationError(
                "Maximum scopes reached".to_string(),
            ));
        }

        let new_engine = Arc::new(RwLock::new(GraphInference::with_config(
    ...
    async fn get_or_create_scope_ordering(
        &self,
        scope: &InferenceScope,
    ) -> Arc<EventOrderingEngine> {
        {
        ...
        let new_engine = Arc::new(EventOrderingEngine::new(
            self.config.ordering_config.clone(),
        ));
    ...
    async fn detect_cross_scope_patterns(
        &self,
        current_scope: &InferenceScope,
        _event: &ScopedEvent,
    ) -> Vec<CrossScopePattern> {
        ...
        if self.config.enable_agent_type_clustering {
            ...
            for similar_scope in similar_scopes {
                if let Some(pattern) = self
                    .compare_scope_patterns(current_scope, &similar_scope)
                    .await
                {
    ...
    async fn find_similar_scopes(&self, target_scope: &InferenceScope) -> Vec<InferenceScope> {
        ...
        engines
            ...
            .filter(|scope| {
                ...
                scope.agent_type == target_scope.agent_type
                    && scope.session_id != target_scope.session_id
            })
            .cloned()
            .collect()
    }
    ...
    async fn compare_scope_patterns(
        &self,
        scope1: &InferenceScope,
        scope2: &InferenceScope,
    ) -> Option<CrossScopePattern> {
    ...
    async fn find_agent_collaboration_patterns(
        &self,
        engine: &Arc<RwLock<GraphInference>>,
    ) -> GraphResult<Vec<CollaborationPattern>> {
        let inference = engine.read().await;
    ...
    async fn find_workflow_sequences(
        &self,
        engine: &Arc<RwLock<GraphInference>>,
    ) -> GraphResult<Vec<WorkflowSequence>> {
        let inference = engine.read().await;
        let patterns = inference.get_temporal_patterns();

        Ok(patterns
            .iter()
            .map(|p| WorkflowSequence {
    ...
    async fn get_scope_agent_types(
        &self,
        engine: &Arc<RwLock<GraphInference>>,
    ) -> GraphResult<Vec<ScopeAgentType>> {
        let inference = engine.read().await;
        let graph = inference.graph();
        let agent_nodes = graph.get_nodes_by_type("Agent");
    ...
    async fn calculate_session_metrics(
        &self,
        engine: &Arc<RwLock<GraphInference>>,
    ) -> GraphResult<SessionMetrics> {
        let inference = engine.read().await;
