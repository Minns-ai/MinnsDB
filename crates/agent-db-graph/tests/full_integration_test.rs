...
fn create_action_event(
    ...
    Event {
        ...
        event_type: EventType::Action {
            ...
            outcome: if success {
                ActionOutcome::Success { result: json!("fixed") }
            } else {
                ActionOutcome::Failure { error: "failed".to_string(), error_code: 500 }
            },
...
async fn test_fully_integrated_self_evolution_pipeline() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, event) in events.iter().enumerate() {
        ...
        println!("  Event {}: {} nodes created, {} patterns",
            i + 1,
    ...
    println!("  Memories automatically formed: {}", memory_stats.total_memories);
    println!("    - Average strength:       {:.2}", memory_stats.avg_strength);
    println!("    - Average access count:   {:.2}", memory_stats.avg_access_count);
    ...
    println!("  Strategies automatically extracted: {}", strategy_stats.total_strategies);
    println!("    - High quality (>0.8):  {}", strategy_stats.high_quality_strategies);
    println!("    - Average quality:      {:.2}", strategy_stats.average_quality);
    ...
    for strategy in &agent_strategies {
        println!("    - Strategy '{}': {} reasoning steps, quality {:.2}",
            strategy.name,
    ...
    println!("  Total patterns:           {}", reinforcement_stats.total_patterns);
    println!("  High confidence (>0.7):   {}", reinforcement_stats.high_confidence_patterns);
    println!("  Average confidence:       {:.2}", reinforcement_stats.average_confidence);
    ...
    println!("  Action suggestions for similar context: {}", suggestions.len());
    ...
    println!("  Nodes created:            {}", engine_stats.total_nodes_created);
    println!("  Episodes detected:        {}", engine_stats.total_episodes_detected);
    println!("  Memories formed:          {}", engine_stats.total_memories_formed);
    println!("  Strategies extracted:     {}", engine_stats.total_strategies_extracted);
    println!("  Reinforcements applied:   {}", engine_stats.total_reinforcements_applied);
    ...
    let all_working =
        engine_stats.total_events_processed > 0 &&
        engine_stats.total_nodes_created > 0;
