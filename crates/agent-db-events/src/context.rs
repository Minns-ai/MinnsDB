...
impl ContextFeatures {
    ...
    fn extract_goal_features(context: &EventContext) -> Vec<f32> {
        ...
        if !context.active_goals.is_empty() {
            let avg_priority: f32 = context.active_goals.iter()
                .map(|g| g.priority)
                .sum::<f32>() / context.active_goals.len() as f32;
            
            let avg_progress: f32 = context.active_goals.iter()
                .map(|g| g.progress)
                .sum::<f32>() / context.active_goals.len() as f32;
            
            let max_priority = context.active_goals.iter()
                .map(|g| g.priority)
            ...
            let goals_with_deadlines = context.active_goals.iter()
    ...
    fn extract_temporal_features(context: &EventContext) -> Vec<f32> {
        vec![
            context.environment.temporal.patterns.len() as f32,
        ]
    }
}
...
impl ContextClassification {
    ...
    pub fn classify(context: &EventContext, features: &ContextFeatures) -> Self {
        ...
        let (primary_type, confidence) = scores.iter()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(t, s)| (t.clone(), *s))
            .unwrap_or((ContextType::Unknown, 0.0));
        
        // Find secondary types (above threshold)
        let secondary_types: Vec<ContextType> = scores.iter()
    ...
    fn score_high_pressure(context: &EventContext, _features: &ContextFeatures) -> f32 {
        ...
        if !context.environment.temporal.deadlines.is_empty() {
            ...
            let high_priority_deadlines = context.environment.temporal.deadlines.iter()
        ...
        // High priority goals
        let high_priority_goals = context.active_goals.iter()
    ...
    fn score_routine(context: &EventContext, _features: &ContextFeatures) -> f32 {
        ...
        let moderate_goals = context.active_goals.iter()
    ...
    fn score_emergency(context: &EventContext, features: &ContextFeatures) -> f32 {
        ...
        let urgent_deadlines = context.environment.temporal.deadlines.iter()
