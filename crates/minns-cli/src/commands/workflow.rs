//! `minns workflow` — create, list, run, and manage workflows via the workflow API.
...
pub async fn execute(client: &MinnsDBClient, args: WorkflowArgs) -> Result<()> {
    match args.command {
...
async fn create(client: &MinnsDBClient, file: &str, group_id: Option<String>) -> Result<()> {
    let content = if file == "-" {
...
async fn list(client: &MinnsDBClient) -> Result<()> {
...
async fn status(client: &MinnsDBClient, workflow_id: u64) -> Result<()> {
...
async fn update(client: &MinnsDBClient, workflow_id: u64, file: &str) -> Result<()> {
...
async fn delete(client: &MinnsDBClient, workflow_id: u64) -> Result<()> {
