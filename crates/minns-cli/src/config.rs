//! Config file loading for `.minns.toml`.
//!
//! Searches up from CWD for `.minns.toml`. Values from the config
...
/// Search up from `start` for `.minns.toml` and parse it.
...
/// Walk up from CWD looking for `.minns.toml`.
fn find_config_file() -> Option<PathBuf> {
    let mut dir = std::env::current_dir().ok()?;
    loop {
        let candidate = dir.join(".minns.toml");
...
/// Get the config file path (for `minns init`).
