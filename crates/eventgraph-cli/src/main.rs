//! EventGraph CLI — cross-session memory and workflow orchestration for EventGraphDB.
//!
//! Usage:
//!   eventgraph recall "how does auth work?"
//!   eventgraph learn "We chose JWT with RS256" --category decision
//!   eventgraph search "database migration" --mode hybrid
//!   eventgraph status
//!   eventgraph workflow create --file workflow.json
//!   eventgraph vibe-graph "Review PR and write tests"
//!   eventgraph scan .
//!   eventgraph code-search --kind function
//!   eventgraph converse "The user prefers dark mode"
//!   eventgraph converse --file conversation.jsonl
//!   eventgraph agents register --repo my-backend
//!   eventgraph agents list
//!   eventgraph init
...
use client::EventGraphClient;

/// EventGraph CLI — cross-session memory and workflow orchestration
#[derive(Parser)]
#[command(name = "eventgraph", version, about)]
struct Cli {
    /// EventGraphDB server URL
    #[arg(long, env = "EVENTGRAPH_URL", default_value = "http://127.0.0.1:3000")]
    url: String,

    /// Group ID for project-scoped isolation
    #[arg(long, env = "EVENTGRAPH_GROUP_ID", default_value = "default")]
    group_id: String,
...
enum Command {
    ...
    /// Initialize a .eventgraph.toml config file in the current directory
    Init {
...
async fn main() -> Result<()> {
    ...
    // Load .eventgraph.toml (searches up from CWD)
    let cfg = config::load_config();

    // Resolve: CLI arg > env var > config file > default
    let url = config::resolve(&cli.url, "EVENTGRAPH_URL", None, "http://127.0.0.1:3000");
    ...
    let client = EventGraphClient::new(url, group_id)?;
...
fn init_config(group_id: Option<String>, agent_id: Option<String>) -> Result<()> {
    ...
    if path.exists() {
        anyhow::bail!(".eventgraph.toml already exists at {}", path.display());
    }
