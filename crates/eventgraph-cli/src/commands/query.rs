//! `eventgraph query` — graph algorithm queries (neighbors, causal-path, communities).
...
pub async fn execute(client: &EventGraphClient, args: QueryArgs) -> Result<()> {
    match args.command {
...
async fn neighbors(client: &EventGraphClient, node_id: u64) -> Result<()> {
...
async fn causal_path(client: &EventGraphClient, from: u64, to: u64) -> Result<()> {
...
async fn communities(client: &EventGraphClient) -> Result<()> {
