...
export function multiply(a, b) {
  return a * b;
    ...
    setupEventListeners() {
        ...
        const taskSelect = document.getElementById('demo-task');
        ...
        refreshButton?.addEventListener('click', () => window.visualizer?.refresh());
        taskSelect?.addEventListener('change', () => this.updateScriptForTask());
    }
    async runAgent(isVariant) {
        if (this.running)
            return;
        this.running = true;
        ...
        const model = document.getElementById('demo-model')?.value.trim() || 'gpt-4o-mini';
        if (!apiKey) {
        ...
        const constraint = document.getElementById('demo-constraint')?.value.trim();
    ...
    getSelectedTask() {
        const selected = document.getElementById('demo-task')?.value;
        return this.tasks.find(task => task.id === selected) || null;
    }
    buildContext(task, isVariant, constraint, script) {
    ...
    async requestAgentPlan(apiKey, model, task, script, constraint, isVariant, prior) {
...
`;
        ...
        try {
            response = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('OpenAI request timed out')), 20000))
            ]);
        }
        catch (error) {
    ...
    extractOutputText(data) {
        ...
        if (Array.isArray(data.output) && data.output.length > 0) {
            ...
            const fallback = content.find((item) => item.text);
            if (fallback?.text)
                return fallback.text;
        }
        return '';
    }
    normalizeJsonOutput(text) {
    ...
    async emitObservation(task, context, script) {
    ...
    async emitReasoning(reasoning, context) {
        await this.sendEvent({
            event_type: {
                Cognitive: {
    ...
    async submitFeedback(success) {
        if (!this.lastContext || !this.lastTask || !this.lastPlan) {
            this.log('Run the agent first.');
            return;
        }
        const feedbackText = document.getElementById('demo-feedback-text')?.value.trim();
    ...
    async fetchPriorKnowledge(task, context, queryId) {
        const summaries = {
            memories: [],
            strategies: []
        };
    ...
    async sendEvent(payload) {
        const now = Date.now();
        const agentId = parseInt(document.getElementById('demo-agent-id')?.value || '1', 10);
        const sessionId = parseInt(document.getElementById('demo-session-id')?.value || '1', 10);
        const event = {
        ...
        try {
            const baseUrl = this.client['baseUrl'] || 'http://127.0.0.1:3000';
    ...
    updateScriptForTask() {
        const task = this.getSelectedTask();
        const scriptEl = document.getElementById('demo-script');
        if (!task || !scriptEl)
            return;
        scriptEl.value = task.script;
    }
    getScriptInput() {
        return document.getElementById('demo-script')?.value.trim() || '';
    }
    applyRevisedScript(script) {
        const scriptEl = document.getElementById('demo-script');
        if (!scriptEl)
            return;
    ...
    formatMemorySummary(memory) {
        ...
        const summaryBits = [];
        if (feedback)
            summaryBits.push(`feedback="${feedback}"`);
        if (constraint)
            summaryBits.push(`constraint="${constraint}"`);
