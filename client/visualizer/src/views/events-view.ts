...
import { EventGraphDBClient, Event as EventGraphEvent } from '../../../dist/index.js';

export class EventsView {
  private client: EventGraphDBClient;
  private events: GraphNode[] = [];
  private filteredEvents: GraphNode[] = [];
  private selectedEvent: GraphNode | null = null;
  ...
  private setupEventListeners() {
    ...
    const typeFilter = document.getElementById('events-filter-type') as HTMLSelectElement;
    const agentFilter = document.getElementById('events-filter-agent') as HTMLSelectElement;
  ...
  private applyFilters() {
    const searchTerm = (document.getElementById('events-search') as HTMLInputElement)?.value.toLowerCase() || '';
    const typeFilter = (document.getElementById('events-filter-type') as HTMLSelectElement)?.value || 'all';
    const agentFilter = (document.getElementById('events-filter-agent') as HTMLSelectElement)?.value || 'all';
    ...
    this.filteredEvents = this.events.filter(event => {
      ...
      // Type filter
      if (typeFilter !== 'all') {
        if (!event.node_type.toLowerCase().includes(typeFilter.toLowerCase())) return false;
      }
  ...
  private renderEventsList() {
    ...
    listContainer.innerHTML = this.filteredEvents.map((event, index) => {
      ...
      const timestamp = new Date(Number(event.created_at) / 1_000_000).toLocaleString();
      ...
      <div class="detail-content">
        <div class="detail-section">
          ...
          <table class="detail-table">
            ...
            <tr><td>Created</td><td>${new Date(Number(event.created_at) / 1_000_000).toLocaleString()}</td></tr>
  ...
  private getEventType(event: GraphNode): string {
    if (event.node_type.includes('Action')) return 'Action';
    if (event.node_type.includes('Observation')) return 'Observation';
    if (event.node_type.includes('Cognitive')) return 'Cognitive';
    return 'Event';
  }

  private getEventTitle(event: GraphNode): string {
    return event.label || event.node_type;
  }

  private getEventOutcome(event: GraphNode): string {
  ...
  private loadSampleIntoEditor() {
    const payload = document.getElementById('events-payload') as HTMLTextAreaElement | null;
    if (!payload) return;
    payload.value = JSON.stringify(this.buildSampleEvent(), null, 2);
    this.setSendStatus('', false);
  }

  private async sendEventFromEditor() {
    const payload = document.getElementById('events-payload') as HTMLTextAreaElement | null;
    if (!payload) return;
  ...
  private setSendStatus(message: string, isError: boolean) {
  ...
  private normalizeEvent(raw: any): EventGraphEvent {
    const now = Date.now();
    const event = raw as EventGraphEvent;
    if (!event.id) event.id = now;
    if (!event.timestamp) event.timestamp = now * 1_000_000;
    if (!event.agent_id) event.agent_id = 1;
    if (!event.agent_type) event.agent_type = 'studio';
    if (!event.session_id) event.session_id = 1;
    if (!event.causality_chain) event.causality_chain = [];
    if (!event.metadata) event.metadata = {};
    if (!event.context) event.context = this.buildSampleEvent().context;
    return event;
  }

  private buildSampleEvent(): EventGraphEvent {
    const now = Date.now();
    return {
      id: now,
  ...
  private updateLayout(hasDetails: boolean) {
