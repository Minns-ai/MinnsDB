...
use agent_db_core::types::{AgentId, AgentType, SessionId, generate_event_id, current_timestamp};
...
use agent_db_graph::scoped_inference::{ScopedInferenceEngine, ScopedInferenceConfig, ScopedEvent, ScopeMetadata, InferenceScope, ScopeQuery};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (scenario_name, events) in scenarios {
        ...
        for event in events {
            ...
            println!("  ✅ Processed [{}/{}]: {} nodes, {} relationships", 
                     event.agent_type, 
            ...
            if !result.cross_scope_patterns.is_empty() {
                println!("    🔗 Cross-scope patterns: {}", result.cross_scope_patterns.len());
            }
        }
        println!();
    }
    ...
    let collaboration_result = engine.query_scope(&coding_scope, ScopeQuery::AgentCollaboration).await?;
    println!("🤝 Coding Assistant Collaborations: {:?}", collaboration_result);
    
    let workflow_result = engine.query_scope(&coding_scope, ScopeQuery::WorkflowSequences).await?;
    ...
    let metrics_result = engine.query_scope(&analyst_scope, ScopeQuery::SessionMetrics).await?;
    println!("📈 Data Analysis Metrics: {:?}", metrics_result);

    // Cross-scope insights
    println!("\n🌐 Cross-Scope Insights:");
    let insights = engine.get_cross_scope_insights().await;
    println!("  Common Agent Patterns: {} types", insights.common_agent_patterns.len());
    println!("  Session Similarities: {} pairs", insights.session_similarities.len());
    println!("  Global Workflows: {} patterns", insights.global_workflows.len());
    ...
    for (agent_type, type_stats) in &stats.stats_by_agent_type {
        println!("  {}: {} sessions, {} agents, {} events",
                 agent_type, 
                 type_stats.session_count,
                 type_stats.unique_agents,
                 type_stats.total_events);
    }
...
async fn create_realistic_scenarios() -> Vec<(String, Vec<ScopedEvent>)> {
    ...
    vec![
        ...
        ("Feature Development Team".to_string(), create_coding_session_events(&base_context).await),
        
        // Scenario 2: Data Analysis Session - Report Generation
        ("Data Analysis Project".to_string(), create_data_analysis_events(&base_context).await),
        
        // Scenario 3: Task Management Session - Project Coordination
        ("Project Management".to_string(), create_task_management_events(&base_context).await),
        
        // Scenario 4: Different Coding Session - Bug Fixing
        ("Bug Fix Session".to_string(), create_bug_fix_events(&base_context).await),
    ]
}
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Requirements analyzed") },
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Feature implemented") },
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Review completed") },
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Dataset loaded") },
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Insights generated") },
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Milestone created") },
                    duration_ns: 500_000,
                },
                causality_chain: Vec::new(),
                context: context.clone(),
        ...
        ScopedEvent {
            event: Event {
                ...
                event_type: EventType::Action {
                    ...
                    outcome: ActionOutcome::Success { result: serde_json::json!("Bug identified") },
