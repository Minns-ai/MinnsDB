...
use agent_db_events::{
    ...
    EventContext, EventType, Goal, ResourceState, TemporalContext,
};
use agent_db_storage::{StorageConfig, StorageEngine};
use serde_json::json;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for batch in 0..(test_events / batch_size) {
        ...
        println!(
            "  Batch {} completed: {} events in {:.2?} ({:.0} events/sec)",
            batch + 1,
            batch_size,
            batch_duration,
    ...
    println!(
        "  Creating {} additional events for recovery test...",
    ...
    println!(
        "  Retrieval throughput: {:.0} events/sec",
        retrieval_throughput
    );
    println!(
        "  Total operations: {} stores, {} retrievals",
        test_events + recovery_test_events,
        retrieved_count
    );
