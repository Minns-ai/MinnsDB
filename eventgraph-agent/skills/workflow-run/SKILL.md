...
Execute a previously created workflow with step-by-step orchestration, tracking progress through EventGraphDB's graph.
...
1. **Load workflow state**:
   ```bash
   eventgraph workflow status <workflow_id>
   ```
...
3. **For each ready step**, transition it to `running`:
   ```bash
   eventgraph workflow step-transition <workflow_id> <step_id> --state running
   ```
...
5. **Report completion**:
   ```bash
   eventgraph workflow step-transition <workflow_id> <step_id> --state completed --result "<summary of what was done>"
   ```
   Or on failure:
   ```bash
   eventgraph workflow step-transition <workflow_id> <step_id> --state failed --result "<what went wrong>"
   ```
...
7. **Capture outcome**:
   ```bash
   eventgraph learn "Workflow completed: <summary>" --category workflow_result --tags workflow
   ```
